...
export default function AdminDashboard() {
  const [stats, setStats] = useState<Stats>({
    ...
    declined: 0, completed: 0, todayCount: 0,
  });
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: appts } = await supabase
        .from("appointments")
        .select("id, service, status, requested_date, created_at, patients(first_name, last_name, phone_mobile)")
      ...
      if (appts) {
        setStats({
          ...
          todayCount: appts.filter((a) => a.requested_date === today).length,
        });
        setRecent(appts.slice(0, 8));
      }
      setLoading(false);
    }
    load();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-teal-700 text-white px-6 py-4 flex items-center justify-between shadow-lg">
        ...
        <div className="flex items-center gap-4">
          <Link href="/admin/appointments" className="text-teal-200 hover:text-white text-sm font-medium transition-colors">Appointments</Link>
          <Link href="/" target="_blank" className="text-teal-200 hover:text-white text-sm font-medium transition-colors">View Site ↗</Link>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Today's Schedule", href: `/admin/appointments?date=${new Date().toISOString().split("T")[0]}`, emoji: "📅" },
            ...
            >
              ...
              <p className="text-sm font-semibold text-gray-700 group-hover:text-teal-600 transition-colors">{action.label}</p>
            </Link>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <Link href="/admin/appointments" className="text-teal-600 text-sm font-medium hover:text-teal-700 transition-colors">
              View all →
            </Link>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-400 text-sm">Appointments will appear here once patients book.</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 text-left">
                    ...
                    <th className="px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-widest">Date</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {recent.map((appt) => (
                    <tr key={appt.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <p className="text-gray-700 text-sm max-w-[180px] truncate">{appt.service}</p>
                      </td>
                      <td className="px-6 py-4">
